...
df_compras = pd.merge(left=df_compras,
                      right=df_produtos[["preço", "produto"]],
                      on="produto",
                      how="left"
                      )
df_compras = df_compras.set_index("data")

df_compras["comissão"] = df_compras["preço"] * 0.05
...
valor_compras = df_compras_filter["preço"].sum()
valor_compras = f"R$ {valor_compras:.2f}"
col1.metric("Valor de compras no período", valor_compras)
col2.metric("Quantidade de compras no período", df_compras_filter["preço"].count())
...
valor_compras_loja = df_compras_filter[df_compras_filter["loja"] == principal_loja]["preço"].sum()
valor_compras_loja = f"R$ {valor_compras_loja:.2f}"
quantidade_compras_loja = df_compras_filter[df_compras_filter["loja"] == principal_loja]["preço"].count()
...
valor_compras_vendedor = df_compras_filter[df_compras_filter["vendedor"] == principal_vendedor]["preço"].sum()
valor_compras_vendedor = f"R$ {valor_compras_vendedor:.2f}"
valor_comissao_vendedor = df_compras_filter[df_compras_filter["vendedor"] == principal_vendedor]["comissão"].sum()
